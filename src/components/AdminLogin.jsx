      ...
      <div className="relative w-full max-w-sm">
        ...
        <div className="text-center mb-12">
          <a href="/" className="inline-flex flex-col items-center gap-1">
            <span className="font-display text-3xl font-bold text-stone-900">PAOLO</span>
            <span className="text-[10px] tracking-[0.5em] text-gold-500 uppercase">Rent a Car</span>
          </a>
          <p className="text-stone-400 text-xs tracking-[0.2em] uppercase mt-6">Panel de Administración</p>
        </div>
        ...
        <div className="bg-white border border-stone-100 p-8">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h1 className="text-stone-900 text-sm tracking-[0.15em] uppercase">Acceso Restringido</h1>
          </div>

          <form onSubmit={handleSubmit} className="space-y-8">
            <div>
              <label className="block text-stone-400 text-xs tracking-[0.2em] uppercase mb-3">
                Contraseña
              </label>
              <input
                type="password"
                ...
                placeholder="••••••••"
                className={`w-full bg-transparent border-b pb-3 pt-2 text-stone-900 placeholder-stone-400 focus:outline-none transition-colors duration-300 ${
                  error ? 'border-red-500' : 'border-stone-300 focus:border-gold-500'
                }`}
                autoFocus
              />
              {error && (
        ...
        <div className="text-center mt-6">
          <button
            onClick={() => navigate('/')}
            className="text-stone-400 hover:text-gold-400 text-xs tracking-[0.15em] uppercase transition-colors duration-300"
          >
            ← Volver al inicio
          </button>
        </div>
